...
import { discussionMeetings, commonComplaints } from '../src/general-physio-data.js';
import { reasoningGuides } from '../src/clinical-reasoning-data.js';
...
if (functionalProblems.length < 20) failures.push(`Problemas: ${functionalProblems.length} < 20`);
if (discussionMeetings.length !== 9) failures.push(`Encontros: ${discussionMeetings.length} != 9`);
if (commonComplaints.length < 10) failures.push(`Queixas comuns: ${commonComplaints.length} < 10`);
if (reasoningGuides.length < 20) failures.push(`Condutas: ${reasoningGuides.length} < 20`);
...
console.log(JSON.stringify({ exercises:exercises.length, muscles:muscles.length, tests:clinicalTests.length, goniometry:goniometry.length, reflexes:reflexes.length, scales:scales.length, neuro:exercises.filter(x=>x.kind==='neuro').length, functionalProblems:functionalProblems.length, discussionMeetings:discussionMeetings.length, commonComplaints:commonComplaints.length, reasoningGuides:reasoningGuides.length }, null, 2));
